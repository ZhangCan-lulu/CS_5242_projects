...
class MultiStageModel(nn.Module):
    def __init__(self, args, num_classes,num_coa_class):
        ...
        self.coar_mode=args.coarse_mode#early,late,both,none
        ...
        self.coarse_label=args.coarse_label
        # ====== main architecture ====== #
        ...
        #elif self.coar_mode=="late":
            #self.stage1 = SingleStageModel(num_layers, num_f_maps, dim_in, num_classes, self.DA_ens)
            #self.stage_coarse_late = SingleStageModel(num_layers, num_f_maps, num_f_maps, num_classes, self.DA_ens)
        #elif self.coar_mode=="all":
            #self.stage_coarse = SingleStageModel(num_layers, num_f_maps, dim_in, num_coa_class, self.DA_ens)
            #self.stage1 = SingleStageModel(num_layers, num_f_maps, num_f_maps, num_classes, self.DA_ens)
            #self.stage_coarse_late = SingleStageModel(num_layers, num_f_maps, num_f_maps, num_classes, self.DA_ens)
        #else:
    ...
    def forward(self, x_s, x_t, mask_s, mask_t, beta, reverse):
        ...
        return pred_source, prob_source, feat_source, pred_target, prob_target, feat_target, \
               ...
               pred_source_2, prob_source_2, pred_target_2, prob_target_2,coa_source_labels,coa_target_labels



    def forward_domain(self, x, mask, domain_GT, beta, reverse):
        #out_feat = self.stage1(x)  # out_feat: (batch, dim, frame#)
        coa_out,coa_out_feat=None,None
        coa_out2, coa_out_feat2 = None, None
        if self.coar_mode=="early" or self.coar_mode=="all":
            coa_out_feat = self.stage_coarse(x)
            if reverse:  # reverse the gradient
                coa_out_feat = GradRevLayer.apply(coa_out_feat, beta[0])

            coa_out = self.stage_coarse.conv_out(coa_out_feat)  # out: (batch, class#, frame#)
            x=coa_out_feat
        ...
        ##
        #out_d_coa, out_d_video_coa, lb_d_coa, lb_d_video_coa, out_feat_video_coa \
        #    = self.forward_stage(coa_out_feat, prob_coa, beta, mask, domain_GT)
        ...
        ##
        #outputs_coa=coa_out#.unsqueeze(1)
        #prob_coas=prob_coa#.unsqueeze(1)
        ##
        ...
        ##
        #outputs_d_coa = out_d_coa.unsqueeze(1)  # (batch x frame#, stage#, class#, 2)
       # outputs_d_video_coa = out_d_video_coa.unsqueeze(1)  # (batch x seg#, stage#, 2)
        #labels_d_coa = lb_d_coa.unsqueeze(1)  # (batch x frame#, stage#, class#)
        #labels_d_video_coa = lb_d_video_coa.unsqueeze(1)  # (batch x seg#, stage#)
        ##
        ...
        return outputs, probs, outputs_feat, outputs_feat_video, outputs_d, outputs_d_video, labels_d, labels_d_video, outputs_2, probs_2,(coa_out,coa_out2)#(coa_out,coaprob_coa,coa_out_feat, out_feat_video_coa)
