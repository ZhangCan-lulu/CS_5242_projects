...
from predict import *
from batch_gen import BatchGenerator#,train_generator,test_generator
import os
import argparse
import random
import time
...
parser.add_argument('--weighted_domain_loss', default='N', type=str, help='weighted domain loss for class-wise domain discriminators')
...
# config & setting
parser.add_argument('--path_data', default='/home/zhangc/data/preject_data/SSTDA_data/Datasets/action-segmentation/')
...
num_coa_class=len(coa_actions_dict)
# initialize model & trainer
model = MultiStageModel(args, num_classes,num_coa_class)
trainer = Trainer(num_classes,num_coa_class)
...
if args.action == "train":
    batch_gen_source = BatchGenerator(num_classes, actions_dict,coa_actions_dict, gt_path, features_path, sample_rate,device)
    batch_gen_target = BatchGenerator(num_classes, actions_dict,coa_actions_dict, gt_path, features_path, sample_rate,device)
    ...
    # train_dataset=train_generator(num_classes, actions_dict, coa_actions_dict, gt_path, features_path,vid_list_file, sample_rate)
    # test_dataset = test_generator(num_classes, actions_dict, coa_actions_dict, gt_path, features_path, vid_list_file_target,
    #                                 sample_rate)
    trainer.train(model, model_dir, results_dir, batch_gen_source,batch_gen_target, device, args)

if args.action == "predict":
    predict(model, model_dir, results_dir, features_path, vid_list_file_test, args.resume_epoch, actions_dict,
