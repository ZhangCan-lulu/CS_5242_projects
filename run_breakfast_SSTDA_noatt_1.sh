...
path_model=models/ # ../../Models_SSTDA/action-segmentation-DA/  user-defined (will create if not existing)
path_result=results/ #../../Results_SSTDA/action-segmentation-DA/ # user-defined (will create if not existing)

# === Config & Setting === #
use_target=uSv # none | uSv
dataset=breakfast # gtea | 50salads | breakfast
use_best_model=target # none | source
num_seg=2 # number of segments for each video (SSTDA: 2 | VCOP: 3 | others: 1)

# SSTDA
DA_adv=rev_grad # none | rev_grad
DA_adv_video=rev_grad #rev_grad_ssl # none | rev_grad_ssl
...
# MSTN
method_centroid=prob_hard # none | prob_hard

# JAN
DA_dis=JAN # none | JAN

# MCD / SWD
DA_ens=MCD # none | MCD | SWD
