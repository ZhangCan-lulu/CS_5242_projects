...
class MultiStageModel(nn.Module):
    ...
    def forward(self, x_s, x_t, mask_s, mask_t, beta, reverse):
        ...
        if 'rev_grad_ssl' in self.DA_adv_video and self.use_target != 'none':
            ...
            # tmp=feat_source_video[0]
            feat_video = torch.cat((feat_source_video[0].unsqueeze(0), feat_target_video), -1)  # (batch, stage#, dim, seg#x2)
            num_batch = feat_video.size(0)
            pred_d_video_ssl_single, label_d_video_ssl_single = self.predict_domain_video_ssl(feat_video.squeeze(0), list_label_d, beta[1])
            ...
            for i in range(1, num_batch):
                feat_video = torch.cat((feat_source_video[i].unsqueeze(0), feat_target_video), -1)  # (batch, stage#, dim, seg#x2)
                pred_d_video_ssl_single, label_d_video_ssl_single = self.predict_domain_video_ssl(feat_video[i].squeeze(0), list_label_d, beta[1])
