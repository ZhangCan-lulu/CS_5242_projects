...
class Trainer:
    def __init__(self, num_classes,num_coa_class):
        self.ce = nn.CrossEntropyLoss(ignore_index=-100)
        self.ce_d = nn.CrossEntropyLoss(reduction='none')

        self.mse = nn.MSELoss(reduction='none')
        self.num_classes = num_classes
        self.coa_classes=num_coa_class
    ...
    def train(self, model, model_dir, results_dir, batch_gen_source, batch_gen_target, device, args):
        ...
        for epoch in range(resume_epoch, num_epochs):
            ...
            while batch_gen_source.has_next():
                ...
                if use_target != 'none':
                    ...
                    if DA_ens != 'none':  # get multiple target outputs
                        _, _, _, _, prob_target, _, _, _, _, _, _, _, _, prob_target_2,_,_ \
            ...
            if verbose:
                ...
                writer.add_scalars("Acc_train",{"Train_acc":acc_epoch_source,"Test_acc":acc_epoch_target},epoch)
                # writer.add_scalar("Acc_test", acc_epoch_target, epoch)
                writer.add_scalar("Loss", epoch_loss / num_iter_epoch, epoch)
